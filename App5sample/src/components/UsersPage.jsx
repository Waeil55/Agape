...
const UsersPage = ({ drivers = [], setDrivers, dispatchers = [], setDispatchers, addAuditLog, currentUser, role, requestAuthAction }) => {
  ...
  const createUser = async () => {
    ...
    try {
      ...
      if (form.role === 'dispatcher') {
        ...
        setDispatchers(prev => [...prev, { id, name: form.email, email: form.email }]);
        ...
        const newDriver = {
          ...
          assignedTo: 'DSP-01', schedule: [], clockedIn: false
        };
  ...
  const assignDriver = (driverId, dispatcherId) => {
    ...
    addAuditLog('Driver Assigned', `${currentUser} reassigned a driver to ${dispatcherId}`, 'blue');
    setShowAssign(null);
  };
