...
const DriverPage = ({ currentUser, role, drivers, trips, activeMission, onUpdateMission, onUpdateTrip, onDriverStatusUpdate, onCompleteTrip, onOpenSettings, appSettings = {}, phoneNumbers = {}, onUpdateDriverLocation, onUpdateAppSettings }) => {
  ...
  return (
    <div className="flex-1 flex flex-col bg-[#f5f5f7]">
      {/* ===== TRIPS PAGE ===== */}
      {activeNav === 'trips' && (
        <div className="flex-1 overflow-y-auto pb-28 px-1.5 pt-1.5 space-y-2">
          {/* Offline Banner */}
          {!isOnline && (
              ...
              {orderedTrips.map((trip) => {
                ...
                return (
                  <div key={trip.id}
                    ...
                    {/* Expanded Details */}
                    {isExpanded && (
                      <div className="px-4 pb-2.5 border-t border-slate-100 pt-3 space-y-3 animate-in">
                    ...
                    {/* Action Buttons */}
                    <div className="px-4 pb-4 flex gap-2.5 flex-wrap">
      ...
      {showOdometerPrompt && (
        <div className="fixed inset-0 z-[120] bg-black/40 backdrop-blur-sm flex items-center justify-center p-6">
          <div className="bg-white rounded-2xl w-full max-w-sm p-5 shadow-2xl relative z-10 border border-white/20">
      ...
      {showTripDetails && (
        <div className="fixed inset-0 z-[130] bg-white flex flex-col animate-slide-up">
          <div className="px-4 py-3 bg-white border-b border-slate-100 flex items-center gap-3 shrink-0">
            <button onClick={() => setShowTripDetails(null)} className="w-8 h-8 rounded-lg bg-slate-100 text-slate-600 flex items-center justify-center active:scale-90"><ChevronLeft size={18} /></button>
            <div className="flex-1">
          ...
          <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-blue-600/90 to-indigo-700/90 shadow-lg shadow-blue-600/10">
            ...
            <div className="relative px-5 py-5">
              <div className="flex items-center gap-4">
                <div className="w-14 h-14 rounded-xl bg-white/20 backdrop-blur-md flex items-center justify-center text-xl font-black text-white shadow-inner border border-white/10">
                  {me?.name?.charAt(0)}
                </div>
                <div className="flex-1 min-w-0">
      ...
      {passwordPrompt && (
        <div className="fixed inset-0 z-[150] bg-black/40 flex items-center justify-center p-6">
          <div className="bg-white rounded-2xl w-full max-w-sm p-5 shadow-2xl relative z-10">
            <div className="text-center mb-5">
