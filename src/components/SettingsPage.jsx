...
const CostOverrideSettings = ({ settings, onSave, saving }) => {
  ...
  return <div className="space-y-5">
    <div><h3 className="text-heading mb-1 text-slate-900">Cost override collection</h3><p className="text-body text-slate-500">Choose which individual booking legs qualify. Directional exclusions apply only in the direction entered.</p></div>
    <div className="rounded-xl border border-blue-200 bg-blue-50 p-4 text-sm font-semibold text-blue-900">Every Booking ID remains independent. These rules never merge outbound, return, or IN/OUT legs.</div>
    ...
    <div className="grid grid-cols-1 gap-4 rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:grid-cols-2 lg:grid-cols-3">
      {numberField('minimumUnloadedMiles', 'Minimum unloaded mileage', 'mi', 0.1)}
      {numberField('unloadedRatePerMile', 'Unloaded mileage rate', '$/mi', 0.01)}
      {numberField('minimumWaitingMinutes', 'Minimum total waiting time', 'min', 1)}
      {numberField('waitingGraceMinutes', 'Waiting time deducted', 'min', 1)}
      {numberField('waitingRatePerHour', 'Waiting rate', '$/hr', 0.01)}
    </div>
    <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
      <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm"><label className="flex min-h-11 items-center gap-3 text-sm font-semibold text-slate-700"><input type="checkbox" checked={Boolean(draft.excludeSameCityUnloaded)} onChange={event => setDraft(current => ({ ...current, excludeSameCityUnloaded: event.target.checked }))} className="h-5 w-5 rounded" />Exclude same-city unloaded mileage</label><label className="mt-3 block text-xs font-semibold text-slate-600">Directional unloaded exclusions — one per line</label><textarea value={pairsToText(draft.unloadedExcludedCityPairs)} onChange={event => setDraft(current => ({ ...current, unloadedExcludedCityPairs: textToPairs(event.target.value) }))} rows="6" placeholder={'Indianapolis > Indianapolis\nCarmel > Fishers'} className="mt-1.5 w-full rounded-xl border border-slate-200 p-3 font-mono text-sm outline-none focus:border-blue-500" /><p className="mt-2 text-xs font-semibold text-slate-400">Use “From city &gt; To city”. Reverse direction is a separate rule.</p></div>
      <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm"><label className="flex min-h-11 items-center gap-3 text-sm font-semibold text-slate-700"><input type="checkbox" checked={Boolean(draft.excludeSameCityWaiting)} onChange={event => setDraft(current => ({ ...current, excludeSameCityWaiting: event.target.checked }))} className="h-5 w-5 rounded" />Exclude same-city waiting</label><label className="mt-3 block text-xs font-semibold text-slate-600">Directional waiting exclusions — one per line</label><textarea value={pairsToText(draft.waitingExcludedCityPairs)} onChange={event => setDraft(current => ({ ...current, waitingExcludedCityPairs: textToPairs(event.target.value) }))} rows="6" placeholder={'Indianapolis > Beech Grove\nFishers > Indianapolis'} className="mt-1.5 w-full rounded-xl border border-slate-200 p-3 font-mono text-sm outline-none focus:border-blue-500" /><p className="mt-2 text-xs font-semibold text-slate-400">Waiting exclusions do not affect unloaded mileage qualification.</p></div>
    </div>
