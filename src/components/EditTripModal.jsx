...
const EditTripModal = ({ trip, onClose, onUpdate, drivers, onSave, driverMode }) => {
  ...
  return (
    <div className="fixed inset-0 z-[120] flex items-center justify-center p-3">
      ...
      <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl relative z-10 border border-slate-200 max-h-[90vh] overflow-hidden flex flex-col">
          ...
          <form id="tripEditForm" onSubmit={handleUpdate} className="space-y-3.5">
            <div className="bg-slate-50 rounded-xl px-3.5 py-3 border border-slate-200">
              <div className="flex items-center justify-between gap-2">
                ...
                {editTrip.bookingId && <span className="text-[11px] font-mono font-bold text-blue-600 bg-blue-50 px-2 py-0.5 rounded shrink-0">Trip: {editTrip.bookingId}</span>}
              </div>
              <p className="text-[11px] text-slate-500 mt-0.5">{editTrip.time || ''} &middot; {editTrip.date || ''}</p>
            </div>

            <div>
              <label className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-1 block flex items-center gap-1">
            ...
            <div className="bg-white border border-slate-200 rounded-xl px-3.5 py-3">
              <label className="flex items-center gap-3 cursor-pointer select-none">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-slate-500">Paper signature / waiver completed</p>
                </div>
              </label>
            </div>
              ...
              <div className="bg-white border border-emerald-200 rounded-xl px-3.5 py-3">
                <label className="flex items-center gap-3 cursor-pointer select-none">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-slate-500">Trip moves to history area after save</p>
                  </div>
                </label>
              </div>
            )}
            ...
            <div>
              <label className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-1 block">Notes</label>
              <textarea value={editTrip.notes || ''} onChange={(e) => handleField('notes', e.target.value)} className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl font-semibold text-sm focus:border-blue-500 focus:bg-white outline-none transition-all" rows="2" placeholder="Update notes..." />
            </div>

            {driverMode && (
              <div className="bg-amber-50 border border-amber-200 rounded-xl p-2.5 flex items-center gap-2.5">
                <Lock size={14} className="text-amber-600 shrink-0" />
                <p className="text-[11px] font-semibold text-amber-800">Password required to save changes.</p>
              </div>
            )}
          </form>
        </div>
