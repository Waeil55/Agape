...
const DesktopReportsPage = ({ trips = [], drivers = [], onUpdateTrip, role, setEditTrip }) => {
      ...
      <div className="relative mx-auto flex min-h-0 w-full flex-1 flex-col overflow-hidden bg-white shadow-sm">
        ...
        <div className="px-4 py-3 flex items-center justify-between bg-white shrink-0">
          <div className="flex min-w-0 items-center gap-1">
            ...
            <button onClick={() => shiftDate(1)} className="p-1 border border-slate-200 rounded text-slate-300 hover:bg-slate-50 transition-colors">
          ...
          <div className="flex items-center gap-1 hidden sm:flex">
            ...
            <button className="p-1.5 border border-slate-200 text-slate-500 rounded-full hover:bg-slate-50"><CheckCircle2 className="w-4 h-4" /></button>
            <button className="p-1.5 border border-slate-200 text-slate-500 rounded-full hover:bg-slate-50"><XCircle className="w-4 h-4" /></button>
            <button className="p-1.5 border border-slate-200 text-slate-500 rounded-full hover:bg-slate-50"><AlertTriangle className="w-4 h-4" /></button>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto bg-slate-50 pb-24">
          ...
          {/* DAILY SUMMARY BAR */}
          <div className="bg-white px-4 py-3 mb-3 border-b border-slate-200 flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-sm">
            <div className="flex flex-wrap items-center gap-2 text-[11px] font-bold text-slate-600">
              <span className="bg-slate-100 px-2 py-1 rounded-md border border-slate-200 text-slate-800 shadow-sm">{filteredTrips.length} trips</span>
              <span className="bg-slate-100 px-2 py-1 rounded-md border border-slate-200 text-slate-800 shadow-sm">{filteredTrips.filter(t => t.reviewed).length}/{filteredTrips.length} reviewed</span>
            </div>
            <button 
              className="flex items-center justify-center gap-1.5 bg-[#e8f7f0] text-emerald-700 border border-emerald-200 rounded-lg px-4 py-2 text-sm font-bold shadow-sm hover:bg-[#d1f0e1] transition-colors"
              onClick={() => {
            ...
            {filteredTrips.map(trip => {
              ...
              return (
                <div key={trip.id} className="rounded-xl shadow-sm overflow-hidden border border-slate-200 bg-white transition-all duration-200 hover:shadow-md hover:border-slate-300">
                  {/* Card Header */}
                  <div 
                    className="bg-[#2b4c7e] px-4 py-3 flex items-center justify-between cursor-pointer select-none transition-colors hover:bg-[#203a60]"
                    onClick={() => toggleExpand(trip.id)}
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      <div className="w-10 h-10 bg-white/10 rounded-xl flex items-center justify-center text-white shrink-0 shadow-sm">
                  ...
                  {/* Card Body */}
                  {isExpanded && (
                    <div className="bg-[#eaf0f6] p-4 animate-in fade-in slide-in-from-top-2 duration-200">
                      ...
                      <div className="mt-4 pt-4 border-t border-slate-300/50 flex gap-2">
                        <button 
                          ...
                          className="flex-1 bg-white border border-slate-300 text-slate-700 rounded-lg py-2 text-xs font-bold hover:bg-slate-50 transition-colors flex justify-center items-center gap-1.5 shadow-sm"
                        >
