    ...
    <div className="h-full min-h-0 w-full max-w-full bg-slate-100 font-sans text-slate-700 flex flex-col overflow-hidden max-md:[&_button]:min-h-11">
      ...
      <div className="bg-white border-b border-slate-200 px-3 sm:px-4 lg:px-5 py-2.5 sm:py-3 flex flex-col lg:flex-row items-start lg:items-center justify-between flex-shrink-0 gap-3">
        ...
        <div className="flex items-center gap-2 w-full lg:w-auto overflow-x-auto hide-scrollbar pb-0.5">
          ...
          <button
            ...
            className="min-h-9 flex items-center gap-1 px-2.5 sm:gap-1.5 sm:px-3 py-1.5 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 text-xs sm:text-xs font-bold rounded-xl border border-indigo-200 transition-all flex-shrink-0"
          >
          ...
          {sequence.length > 0 && (
            <button
              ...
              className="min-h-9 min-w-9 p-2 bg-slate-50 border border-slate-200 rounded-xl text-slate-500 hover:text-red-500 hover:bg-red-50 transition-colors flex-shrink-0"
      ...
      <div className="lg:hidden p-2.5 bg-white border-b border-slate-200 flex gap-2 flex-shrink-0">
        <button
          ...
          className={`flex-1 min-h-10 py-2 rounded-xl text-sm font-bold transition-all ${
            ...
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 px-3 sm:px-4 py-3 border-b border-slate-100">
              <div className="flex items-center justify-between sm:justify-start gap-2 min-w-0">
                ...
                <div className="flex bg-slate-100 rounded-lg p-0.5 gap-0.5">
                  <button onClick={() => setActiveLeg('A')} className={`px-2 py-1 text-xs font-bold rounded-md transition-all ${activeLeg === 'A' ? 'bg-white text-indigo-700 shadow-sm border border-indigo-200' : 'text-slate-500 hover:text-slate-600'}`}><Flag className="w-3 h-3 inline mr-0.5" />A</button>
                  <button onClick={() => setActiveLeg('B')} className={`px-2 py-1 text-xs font-bold rounded-md transition-all ${activeLeg === 'B' ? 'bg-white text-amber-700 shadow-sm border border-amber-200' : 'text-slate-500 hover:text-slate-600'}`}><Flag className="w-3 h-3 inline mr-0.5" />B</button>
                </div>
              </div>
              <div className="flex items-center justify-between sm:justify-end gap-2 w-full sm:w-auto">
                {availableToday.length > 0 && (
                  <button
                    ...
                    className="min-h-9 flex flex-1 sm:flex-none items-center justify-center gap-1 px-2.5 py-1 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 text-xs font-bold rounded-lg transition-colors border border-indigo-100"
                ...
                <button
                  ...
                  className="min-h-9 flex flex-1 sm:flex-none items-center justify-center gap-1 px-2.5 py-1 bg-blue-50 hover:bg-blue-100 text-blue-700 text-xs font-bold rounded-lg transition-colors border border-blue-100"
                >
                  ...
                  <div key={client.id} className={`bg-white border shadow-sm rounded-xl p-3 hover:border-indigo-300 transition-all duration-300 group ${
                    ...
                    <div className="flex justify-between items-start mb-2">
                      <div className="min-w-0 flex-1 pr-2">
                        ...
                        <div className="flex flex-wrap gap-1 mt-1">
                           <span className="px-1.5 py-0.5 bg-slate-100 text-slate-700 text-xs font-semibold rounded">{client.req}</span>
                          {client.bookingId && (
                             <span className="px-1.5 py-0.5 bg-blue-50 text-blue-700 text-xs font-semibold rounded border border-blue-100">{client.bookingId}</span>
                          )}
                           <span className={`px-1.5 py-0.5 text-xs font-semibold rounded ${statusColor(client.tripStatus)}`}>
                            {client.tripStatus}
                          </span>
                          {client.isTemp && (
                             <span className="px-1.5 py-0.5 bg-amber-50 text-amber-700 text-xs font-semibold rounded border border-amber-200">ONE-TIME</span>
                          )}
                        </div>
                        {client.driverName && (
                          <p className="text-xs font-semibold text-slate-500 mt-1 truncate">Driver: {client.driverName}</p>
                        )}
                    ...
                    <div className="flex gap-1.5">
                      <button
                        ...
                        className={`min-h-10 flex-1 flex justify-between items-center px-2.5 py-1.5 border rounded-xl text-xs font-bold transition-all ${
                      ...
                      >
                        <span className="flex items-center gap-1">
                           <span className="w-2 h-2 rounded-full bg-emerald-500" /> <span className="uppercase">Pickup</span> <span className="font-semibold">{client.puTime}</span>
                        </span>
        ...
        <div className={`h-full min-h-0 flex-1 flex-col overflow-hidden border-y border-slate-200 bg-white shadow-sm lg:rounded-xl lg:border ${mobileView === 'sequence' ? 'flex' : 'hidden lg:flex'}`}>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1.5 px-3 sm:px-4 py-3 border-b border-slate-100 flex-shrink-0">
            <div className="min-w-0">
              ...
              {initialOrigin && (
                <p className="text-xs font-semibold text-emerald-700 mt-0.5 truncate max-w-full">
                        ...
                        nodes.push(
                          <div key={'leg-label-' + leg} className="flex items-center gap-2 mb-2">
                            ...
                            <span className={'text-xs font-semibold uppercase tracking-wider ' + (leg === 'B' ? 'text-amber-700' : 'text-indigo-700')}>{leg === 'B' ? 'Leg B' : 'Leg ' + leg}</span>
                             {legCompletion[leg] === 'complete' && <span className="text-xs font-semibold text-emerald-600 bg-emerald-100 px-2 py-0.5 rounded-full">All Done</span>}
                             {legCompletion[leg] === 'partial' && <span className="text-xs font-semibold text-amber-600 bg-amber-100 px-2 py-0.5 rounded-full">Partial</span>}
                        ...
                        <div className={`min-w-0 flex-1 bg-white rounded-xl sm:rounded-xl p-2 sm:p-3 shadow-sm flex items-start sm:items-center gap-2 transition-all border ${isPU ? 'border-emerald-100 hover:border-emerald-300' : 'border-red-100 hover:border-red-300'} ${isOverCap ? 'ring-2 ring-red-400' : ''} ${client.urgency === 2 ? 'shadow-[0_0_12px_-2px_rgba(225,29,72,0.4)] border-rose-300' : client.urgency === 1 ? 'shadow-[0_0_10px_-2px_rgba(245,158,11,0.3)] border-amber-300' : ''} ${effectiveOverride ? (effectiveOverride.color === 'rose' ? 'ring-2 ring-rose-300 bg-rose-50/30' : effectiveOverride.color === 'amber' ? 'ring-2 ring-amber-300 bg-amber-50/30' : effectiveOverride.color === 'red' ? 'ring-2 ring-red-400 bg-red-50/30 line-through opacity-60' : effectiveOverride.color === 'emerald' ? 'ring-2 ring-emerald-300 bg-emerald-50/30' : 'ring-2 ring-slate-300 bg-slate-50/30 line-through opacity-60') : ''}`}>
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-1.5 mb-0.5">
                               <span className={`text-xs font-semibold uppercase tracking-wider px-1.5 py-0.5 rounded ${isPU ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'}`}>{isPU ? '⬆ PU' : '⬇ DO'}</span>
                              <span className="text-xs font-semibold text-slate-500">{isPU ? client.puTime : client.doTime}</span>
                               {effectiveOverride && <span className={`px-1.5 py-0.5 rounded text-[8px] font-semibold uppercase tracking-wider ${effectiveOverride.color === 'emerald' ? 'bg-emerald-100 text-emerald-800' : effectiveOverride.color === 'amber' ? 'bg-amber-100 text-amber-800' : effectiveOverride.color === 'rose' ? 'bg-rose-100 text-rose-800' : effectiveOverride.color === 'red' ? 'bg-red-100 text-red-800' : 'bg-slate-100 text-slate-600'}`}>{effectiveOverride.label}</span>}
                            </div>
                            <h4 className="font-semibold text-slate-900 text-sm truncate flex items-center gap-1">
                              ...
                              {client.name}
                               {tripStatus === 'In Progress' && <span className="text-[8px] font-semibold text-blue-700 bg-blue-100 px-1.5 py-0.5 rounded-full ml-auto animate-pulse flex-shrink-0">LIVE</span>}
                            </h4>
                            <div className="mt-1 flex flex-wrap gap-1 items-center">
                               <span className={`rounded-full border px-1.5 py-0.5 text-xs font-semibold ${tripStatus === 'In Progress' ? 'border-blue-200 bg-blue-100 text-blue-800 ring-1 ring-blue-300' : 'border-blue-100 bg-blue-50 text-blue-700'}`}>{client.bookingId || tripIdForDisplay}</span>
                               {client.req && <span className="rounded-full border border-slate-200 bg-slate-100 px-1.5 py-0.5 text-xs font-semibold text-slate-600">{client.req}</span>}
                            </div>
                            <p className="text-xs font-semibold text-slate-500 truncate flex items-center gap-1 mt-0.5"><MapPin className="w-2.5 h-2.5 text-indigo-400 flex-shrink-0" />{isPU ? client.pu : client.do}</p>
                            {client.notes && <p className="mt-1 truncate text-xs font-medium text-amber-700">Notes: {client.notes}</p>}
                          </div>
            ...
            <div className="p-4 sm:p-5 space-y-4 bg-slate-50 overflow-y-auto">
              ...
              <div className="flex bg-white p-1 rounded-xl border border-slate-200">
                <button
                  ...
                  className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${saveType === 'recurring' ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-500'}`}
                >
                  Recurring Template
                </button>
                <button
                  ...
                  className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${saveType === 'today' ? 'bg-amber-500 text-white shadow-md' : 'text-slate-500'}`}
                >
                  Today Override
                </button>
              </div>
                    ...
                    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2">
                      <div className="min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                           <span className={`text-xs uppercase font-semibold px-2 py-0.5 rounded-md border ${normalizeRouteRecord(tpl).statusBadgeClass}`}>
