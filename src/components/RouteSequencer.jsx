                    ...
                    <div className="flex justify-between items-start mb-2">
                      <div className="min-w-0 flex-1 pr-2">
                        ...
                        <div className="flex flex-wrap gap-1 mt-1">
                          <span className="px-1.5 py-0.5 bg-slate-100 text-slate-700 text-xs font-bold rounded">{client.req}</span>
                          {client.bookingId && (
                            <span className="px-1.5 py-0.5 bg-blue-50 text-blue-700 text-xs font-bold rounded border border-blue-100">{client.bookingId}</span>
                          )}
                          <span className={`px-1.5 py-0.5 text-xs font-bold rounded ${statusColor(client.tripStatus)}`}>
                            {client.tripStatus}
                          </span>
                          {client.isTemp && (
                            <span className="px-1.5 py-0.5 bg-amber-50 text-amber-700 text-xs font-bold rounded border border-amber-200">ONE-TIME</span>
                          )}
                        </div>
                        {client.driverName && (
                    ...
                    <div className="flex gap-1.5">
                      ...
                      >
                        <span className="flex items-center gap-1">
                          <span className="w-2 h-2 rounded-full bg-emerald-500" /> <span className="uppercase">Pickup</span> <span className="font-bold">{client.puTime}</span>
                        </span>
                    ...
                    nodes.push(
                      ...
                      >
                        <div className="flex flex-col items-center pt-1">
                          <div className={`w-7 h-7 sm:w-9 sm:h-9 lg:w-10 lg:h-10 rounded-lg sm:rounded-xl flex items-center justify-center font-bold text-xs sm:text-sm border-2 sm:border-4 border-[#f4f7fb] shadow-sm flex-shrink-0 z-10 ${isPU ? 'bg-emerald-500 text-white' : 'bg-rose-500 text-white'}`}>
                            {idx + 1}
                          </div>
                        </div>
                        <div className={`min-w-0 flex-1 bg-white rounded-xl sm:rounded-2xl p-2 sm:p-3 shadow-sm flex items-start sm:items-center gap-2 transition-all border ${isPU ? 'border-emerald-100 hover:border-emerald-300' : 'border-red-100 hover:border-red-300'} ${isOverCap ? 'ring-2 ring-red-400' : ''} ${client.urgency === 2 ? 'shadow-[0_0_12px_-2px_rgba(225,29,72,0.4)] border-rose-300' : client.urgency === 1 ? 'shadow-[0_0_10px_-2px_rgba(245,158,11,0.3)] border-amber-300' : ''} ${effectiveOverride ? (effectiveOverride.color === 'rose' ? 'ring-2 ring-rose-300 bg-rose-50/30' : effectiveOverride.color === 'amber' ? 'ring-2 ring-amber-300 bg-amber-50/30' : effectiveOverride.color === 'red' ? 'ring-2 ring-red-400 bg-red-50/30 line-through opacity-60' : effectiveOverride.color === 'emerald' ? 'ring-2 ring-emerald-300 bg-emerald-50/30' : 'ring-2 ring-slate-300 bg-slate-50/30 line-through opacity-60') : ''}`}>
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="mt-1 flex flex-wrap gap-1 items-center">
                              <span className={`rounded-full border px-1.5 py-0.5 text-xs font-bold ${tripStatus === 'In Progress' ? 'border-blue-200 bg-blue-100 text-blue-800 ring-1 ring-blue-300' : 'border-blue-100 bg-blue-50 text-blue-700'}`}>{client.bookingId || tripIdForDisplay}</span>
                              {client.req && <span className="rounded-full border border-slate-200 bg-slate-100 px-1.5 py-0.5 text-xs font-bold text-slate-600">{client.req}</span>}
                            </div>
