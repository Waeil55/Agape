  ...
  const renderPanelContent = () => {
    switch (activePanel) {
      ...
      case 'settings': return (
        <ErrorBoundary><Suspense fallback={<LazyFallback />}>
        <SettingsPage currentUser={currentUser} role={role} onLogout={onLogout} onResetSystem={() => { setTrips([]); setTrashedTrips([]); setDrivers([]); addAuditLog('System Reset', 'Master data wipe performed by Admin.', 'rose'); }} appSettings={appSettings} onUpdateAppSettings={updateAppSettings} overridePolicy={overridePolicy} overridePolicyStatus={overridePolicyStatus} overridePolicyError={overridePolicyError} updateOverridePolicy={updateOverridePolicy} phoneNumbers={phoneNumbers} onUpdatePhoneNumbers={(updates) => { setPhoneNumbers(prev => ({ ...prev, ...updates })); setTimeout(persistState, 0); }} requestAuthAction={requestAuthAction} hasPermission={hasPermission} driverProfile={null} trips={trips} drivers={drivers} dispatchers={dispatchers} vehicles={vehicles} logs={logs} persistState={persistState} />
        </Suspense></ErrorBoundary>
      );
