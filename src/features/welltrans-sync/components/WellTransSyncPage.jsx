...
import { explainWellTransFailure, queueWellTransSync, saveWellTransSettings } from '../services/welltransService';
...
const WellTransSyncPage = ({ trips = [], role = 'driver' }) => {
  const { settings, logs, loading, completedTrips, readyTrips, latestByTrip } = useWellTransSync(trips);
      ...
      <div className="mx-auto max-w-[1500px] space-y-5">
        <header className="rounded-3xl bg-slate-950 px-5 py-5 text-white shadow-xl sm:px-7">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="flex items-center gap-2 rounded-2xl border border-white/10 bg-white/5 px-4 py-3"><span className={`h-2.5 w-2.5 rounded-full ${settings.enabled ? 'bg-emerald-400' : 'bg-amber-400'}`} /><div><p className="text-[10px] font-bold uppercase text-slate-400">Engine</p><p className="text-xs font-black">{settings.enabled ? 'Enabled' : 'Disabled in Settings'}</p></div></div>
          </div>
        </header>

        <section className="grid grid-cols-2 gap-3 lg:grid-cols-5">{cards.map(([label, value, Icon, color]) => <div key={label} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><div className={`flex h-9 w-9 items-center justify-center rounded-xl ${color}`}><Icon size={17} /></div><p className="mt-3 text-lg font-black text-slate-950">{value}</p><p className="mt-1 text-[10px] font-bold uppercase tracking-wide text-slate-500">{label}</p></div>)}</section>
        ...
        <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_340px]">
          <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            ...
            {loading ? <div className="p-12 text-center"><Loader2 className="mx-auto animate-spin text-blue-600" /></div> : tab === 'queue' ? (
              <div className="overflow-x-auto"><table className="w-full min-w-[850px] text-left"><thead className="bg-white text-[10px] uppercase text-slate-500"><tr>{['', 'Booking ID', 'Passenger', 'Driver', 'Pickup', 'Dropoff', 'Mileage', 'Readiness', 'Sync'].map(label => <th key={label} className="px-3 py-3 font-black">{label}</th>)}</tr></thead><tbody>{completedTrips.map(trip => { const validation = validateTripForWellTrans(trip); const latest = latestByTrip.get(trip.id); return <tr key={trip.id} className="border-t border-slate-100 text-xs"><td className="px-3 py-3"><input type="checkbox" disabled={!validation.valid || latest?.status === 'completed'} checked={selectedIds.includes(trip.id)} onChange={() => setSelectedIds(ids => ids.includes(trip.id) ? ids.filter(id => id !== trip.id) : [...ids, trip.id])} /></td><td className="px-3 py-3 font-mono font-black text-blue-700">{trip.bookingId || trip.id}</td><td className="px-3 py-3 font-bold text-slate-800">{trip.patient || trip.clientName || '—'}</td><td className="px-3 py-3 text-slate-600">{trip.driverName || '—'}</td><td className="px-3 py-3 text-slate-600">{trip.arrivalTime ? 'Captured' : 'Missing'}</td><td className="px-3 py-3 text-slate-600">{trip.arrivalDropoffTime || trip.completedAt ? 'Captured' : 'Missing'}</td><td className="px-3 py-3 text-slate-600">{validation.payload?.dropoff.mileage ?? '—'}</td><td className="px-3 py-3">{validation.valid ? <span className="font-bold text-emerald-700">Ready</span> : <span title={validation.errors.join(', ')} className="font-bold text-rose-700">{validation.errors[0]}</span>}</td><td className="px-3 py-3"><span className={`rounded-full px-2 py-1 text-[10px] font-black uppercase ${statusStyle[latest?.status] || 'bg-slate-100 text-slate-500'}`}>{latest?.status || 'Not queued'}</span></td></tr>; })}</tbody></table></div>
            ) : tab === 'logs' ? (
              <div className="divide-y divide-slate-100">{logs.length ? logs.map(log => <button key={log.id} onClick={() => log.status === 'failed' && setSelectedFailure(log)} className="flex w-full items-center gap-3 p-4 text-left hover:bg-slate-50"><span className={`rounded-full px-2 py-1 text-[10px] font-black uppercase ${statusStyle[log.status]}`}>{log.status}</span><div className="min-w-0 flex-1"><p className="font-mono text-xs font-black text-slate-900">{log.bookingId || log.tripId}</p><p className="truncate text-[11px] text-slate-500">{log.errorMessage || log.stage || 'Synchronization completed'}</p></div><span className="text-[10px] font-semibold text-slate-400">{displayTime(log.completedAt || log.startedAt || log.createdAt)}</span>{log.screenshot && <ExternalLink size={14} />}</button>) : <p className="p-12 text-center text-sm font-semibold text-slate-500">No synchronization history yet.</p>}</div>
            ) : (
