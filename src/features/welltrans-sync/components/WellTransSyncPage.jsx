...
import { confirmWellTransApplied, explainWellTransFailure, isWellTransFailureRetryable, queueWellTransSync, saveWellTransSettings } from '../services/welltransService';
...
const WellTransSyncPage = ({ trips = [], role = 'driver' }) => {
  ...
  const {
    ...
    requiredWorkerVersion, workerStandby, loading, completedTrips, readyTrips, latestByTrip,
  ...
  const successful = currentLogs.filter(log => log.status === 'completed').length;
  const staged = currentLogs.filter(log => log.status === 'awaiting_review').length;
  const failed = currentLogs.filter(log => log.status === 'failed');
  const retryableFailed = failed.filter(isWellTransFailureRetryable);
  const unmatched = failed.length - retryableFailed.length;
  ...
  const cards = [
    ['Selected Date Trips', completedTrips.length, CheckCircle2, 'text-slate-900 bg-slate-50'],
    ...
    <div className="min-h-full bg-slate-50 p-3 sm:p-5 lg:p-7">
      <div className="mx-auto max-w-[1500px] space-y-5">
        ...
        <section className="flex flex-wrap gap-2 rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
          <label className="flex items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2"><span className="text-[10px] font-black uppercase text-slate-500">Service date</span><input type="date" value={syncDate} onChange={event => { setSyncDate(event.target.value); setSelectedIds([]); }} className="bg-transparent text-xs font-black text-slate-900 outline-none" /></label>
          <button disabled={!workerDateMatches || !readyTrips.length || busy} onClick={() => runQueue(readyTrips.map(trip => trip.id), 'selected-date')} className="rounded-xl bg-violet-600 px-4 py-2.5 text-xs font-black text-white disabled:opacity-40">{busy === 'selected-date' ? <Loader2 size={14} className="inline animate-spin" /> : `STAGE SELECTED DATE (${readyTrips.length})`}</button>
          ...
          <button disabled={!workerDateMatches || !retryableFailed.length || busy} onClick={() => runQueue(retryableFailed.map(log => log.tripId), 'retry')} className="rounded-xl border border-slate-200 px-4 py-2.5 text-xs font-black text-slate-700 disabled:opacity-40"><RefreshCw size={14} className="mr-1 inline" /> RETRY FIXABLE ({retryableFailed.length})</button>
          ...
          {workerUpgradeRequired && <p className="flex w-full items-center gap-2 rounded-xl bg-rose-50 px-3 py-2 text-xs font-bold text-rose-800"><AlertTriangle size={14} />Restart worker required. The connected worker is {worker?.version || 'an older version'}; field-complete staging requires {requiredWorkerVersion}. Staging and retry actions are locked until the corrected worker reconnects.</p>}
          ...
          {workerCalibrated && !workerDateMatches && <p className="flex w-full items-center gap-2 rounded-xl bg-rose-50 px-3 py-2 text-xs font-bold text-rose-800"><AlertTriangle size={14} />Agape is set to {syncDate}, but WellTrans is open on {worker.selectedDate}. Restart the worker and select {syncDate} in WellTrans before staging.</p>}
          ...
          {unmatched > 0 && <p className="flex w-full items-center gap-2 rounded-xl bg-amber-50 px-3 py-2 text-xs font-bold text-amber-800"><AlertTriangle size={14} />{unmatched} failed trip{unmatched === 1 ? '' : 's'} cannot be retried automatically because the exact Booking ID does not have one Pickup and one Dropoff row on WellTrans for {syncDate}. These are excluded from bulk retry.</p>}
        ...
        <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_340px]">
          <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            ...
            {loading ? <div className="p-12 text-center"><Loader2 className="mx-auto animate-spin text-blue-600" /></div> : tab === 'queue' ? (
              <div className="overflow-x-auto"><table className="w-full min-w-[850px] text-left"><thead className="bg-white text-[10px] uppercase text-slate-500"><tr>{['', 'Booking ID', 'Passenger', 'Driver', 'Pickup', 'Dropoff', 'Mileage', 'Readiness', 'Sync'].map(label => <th key={label} className="px-3 py-3 font-black">{label}</th>)}</tr></thead><tbody>{completedTrips.map(trip => { const validation = validateTripForWellTrans(trip); const latest = latestByTrip.get(trip.id); const locked = ['pending', 'processing', 'completed', 'awaiting_review'].includes(latest?.status); const unmatchedBooking = latest?.status === 'failed' && !isWellTransFailureRetryable(latest); return <tr key={trip.id} className="border-t border-slate-100 text-xs"><td className="px-3 py-3"><input type="checkbox" disabled={!validation.valid || locked || unmatchedBooking} checked={selectedIds.includes(trip.id)} onChange={() => setSelectedIds(ids => ids.includes(trip.id) ? ids.filter(id => id !== trip.id) : [...ids, trip.id])} /></td><td className="px-3 py-3 font-mono font-black text-blue-700">{trip.bookingId || trip.id}</td><td className="px-3 py-3 font-bold text-slate-800">{trip.patient || trip.clientName || '—'}</td><td className="px-3 py-3 text-slate-600">{trip.driverName || '—'}</td><td className="px-3 py-3 text-slate-600">{trip.arrivalTime ? 'Captured' : 'Missing'}</td><td className="px-3 py-3 text-slate-600">{trip.arrivalDropoffTime || trip.completedAt ? 'Captured' : 'Missing'}</td><td className="px-3 py-3 text-slate-600">{validation.payload?.dropoff.mileage ?? '—'}</td><td className="px-3 py-3">{unmatchedBooking ? <span className="font-bold text-amber-700">Booking not found on selected WellTrans date</span> : validation.valid ? <span className="font-bold text-emerald-700">Ready</span> : <span title={validation.errors.join(', ')} className="font-bold text-rose-700">{validation.errors[0]}</span>}</td><td className="px-3 py-3"><span className={`rounded-full px-2 py-1 text-[10px] font-black uppercase ${statusStyle[latest?.status] || 'bg-slate-100 text-slate-500'}`}>{latest?.status || 'Not queued'}</span></td></tr>; })}</tbody></table></div>
            ) : tab === 'logs' ? (
