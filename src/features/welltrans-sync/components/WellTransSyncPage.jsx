...
const WellTransSyncPage = ({ trips = [], role = 'driver' }) => {
  ...
  useEffect(() => {
    if (worker?.selectedDate) setSyncDate(worker.selectedDate);
  }, [worker?.selectedDate]);
  ...
  const workerDateReady = useMemo(() => readyTrips.filter(trip => {
    ...
    return syncDate && raw.slice(0, 10) === syncDate;
  }), [readyTrips, syncDate]);
    ...
    <div className="min-h-full bg-slate-50 p-3 sm:p-5 lg:p-7">
      <div className="mx-auto max-w-[1500px] space-y-5">
        ...
        <section className="flex flex-wrap gap-2 rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
          ...
          <button disabled={!workerDateMatches || !workerDateReady.length || busy} onClick={() => runQueue(workerDateReady.map(trip => trip.id), 'worker-date')} className="rounded-xl bg-violet-600 px-4 py-2.5 text-xs font-black text-white disabled:opacity-40">{busy === 'worker-date' ? <Loader2 size={14} className="inline animate-spin" /> : `STAGE ${syncDate || 'SELECTED DATE'} (${workerDateReady.length})`}</button>
